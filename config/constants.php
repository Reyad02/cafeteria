<?php 
   ...
   if(empty($_SESSION['id'])){
    ...
    function sqlReturnTotalPrice($c_id){
        return "SELECT sum(quantity*price) as sum FROM cart join food_items on(cart.food_id=food_items.id) where c_id = '$c_id'";
    }
...
function returnTotalPrice($conn,$c_id){
    $sql = sqlReturnTotalPrice($c_id);
...
function sqlReturnYearlyRevenue($year='year(CURRENT_DATE)'){
    return "SELECT nvl(sum(price*quantity),0) as total, year(pickup) as year FROM
    orders join contains on (orders.id=contains.order_id)
    join food_items on(contains.food_id=food_items.id)
    join user on (user.id=orders.c_id)
    where paid = 1
...
function sqlUpdateStockByArray($conn,$arr){
    //$arr = $food_id => $stock
    foreach($arr as $food_id=>$stock){
        $str = "UPDATE food_items set stock = $stock where food_id = $food_id";
        mysqli_query($conn,$str);
    }
