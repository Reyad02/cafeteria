...
<div class="container">
    ...
    color: aliceblue;
    padding: 9px;
    border-radius: 7px;">Order History</h1>
    <h3 style="margin-top: 33px;">Today is <?php echo date("Y-m-d");?></h3>
        ...
        <?php
            ...
            if($count>0)
            {
                while($row=mysqli_fetch_assoc($res))
                {
                    ...
                    if($sellerFlag){
                        $username = $row['username']==null?'GUEST':$row['username'];
                        $aItem = "<strong>".$username."</strong>_order ".$order_id." ( ".$pickup." )  ";
                        if($paid==0){
                            $aItem .="<a class='btn btn-primary' onclick='markAsPaid($order_id)' style='margin-left:10px;'>Mark as Paid</a>";
                        }else{
                            $aItem .=" <em>Paid</em>";
                        }
                    ...
                    if($flag){
                        $aItem .= "<a class='btn btn-danger' onclick='removeOrder($order_id)' style='margin-left: 11px;'>Remove Preorder</a>";
                    }
...
?>
<h2><?php echo $type==4?"Revenue":"Expenditure";?> History for <input type=number value='<?php echo date("Y");?>' onchange='renderExpenditure(this.value)' placeholder="YYYY" min="1999" max="2030"></input></h2>
<?php
